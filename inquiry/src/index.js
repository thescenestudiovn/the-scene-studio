...
var index_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (!data.name?.trim() || !data.email?.trim() || !data.location?.trim() || !data.story?.trim()) {
        return jsonResponse(
          {
      ...
      const partnerName = data.partnerName?.trim() || "Not specified";
      const services = data.services?.length ? data.services.join(", ") : "Not specified";
      const subject = `New Inquiry \u2014 ${data.name}${data.partnerName ? ` & ${data.partnerName}` : ""}`;
      const html = `
				<div style="font-family: Arial, Helvetica, sans-serif; color: #171717; line-height: 1.7;">
					...
					<p>
						<strong>Date:</strong>
						${escapeHtml(
        data.weddingDate || data.dateStatus || "Not specified"
      )}
					</p>
