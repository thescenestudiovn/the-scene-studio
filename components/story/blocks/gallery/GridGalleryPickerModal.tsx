...
type Props = { open: boolean; selectedIds: string[]; onClose: () => void; onDone: (mediaIds: string[], media: Media[]) => void; maxSelection?: number };

export default function GridGalleryPickerModal({ open, selectedIds, onClose, onDone, maxSelection }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setSelected(maxSelection ? selectedIds.slice(0, maxSelection) : selectedIds);
    setLoading(true);
    Promise.all([
  ...
  const toggle = (id: string) => setSelected(current => {
    ...
    if (maxSelection && current.length >= maxSelection) return maxSelection === 1 ? [id] : current;
    return [...current, id];
  });
  const done = () => {
