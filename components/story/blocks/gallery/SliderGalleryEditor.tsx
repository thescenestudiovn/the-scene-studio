...
type ManageProps = { open: boolean; media: Media[]; ids: string[]; onChange: (ids: string[]) => void; onCancel: () => void; onDone: () => void; onAdd: () => void };
...
function ManageImages({ open, media, ids, onChange, onCancel, onDone, onAdd }: ManageProps) {
  ...
  const move = (targetId: string) => {
    ...
    const next = [...ids], from = next.indexOf(dragId), to = next.indexOf(targetId);
    if (from < 0 || to < 0) return;
    next.splice(from, 1); next.splice(to, 0, dragId); onChange(next); setDragId(null); setOverId(null);
  };
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[2px]">
    <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      <div className="flex shrink-0 items-center justify-between border-b border-[#ebe7e0] px-6 py-5"><div><p className="text-[10px] uppercase tracking-[.18em] text-[#8a857d]">Slider Gallery</p><h2 className="mt-1 font-serif text-2xl text-[#171717]">Manage images</h2></div><button type="button" onClick={onCancel} className="flex h-9 w-9 items-center justify-center rounded-full text-xl text-[#77736c] hover:bg-[#f5f2ed]">×</button></div>
...
function SlideshowPreview({ media }: { media: Media[] }) {
  ...
  useEffect(() => { setIndex(value => Math.min(value, Math.max(media.length - 1, 0))); }, [media.length]);
  useEffect(() => { if (media.length < 2) return; const timer = window.setInterval(() => setIndex(value => (value + 1) % media.length), 3500); return () => window.clearInterval(timer); }, [media.length]);
  if (!media.length) return null;
  const current = media[index] ?? media[0];
  return <div className="relative overflow-hidden bg-[#e9e5de]">
    <img src={mediaUrl(current.path)} alt={current.alt ?? current.filename ?? ""} className="pointer-events-none block h-[420px] w-full object-cover sm:h-[500px]" />
    {media.length > 1 && <>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex items-center justify-between bg-gradient-to-t from-black/55 to-transparent px-5 pb-4 pt-10 text-white"><span className="text-[10px] uppercase tracking-[.16em]">{String(index + 1).padStart(2, "0")} / {String(media.length).padStart(2, "0")}</span><div className="flex gap-1.5">{media.map((item, itemIndex) => <button key={item.id} type="button" aria-label={`Go to image ${itemIndex + 1}`} onClick={event => { event.stopPropagation(); setIndex(itemIndex); }} className={`h-1.5 w-7 transition ${itemIndex === index ? "bg-white" : "bg-white/45"}`} />)}</div></div>
    </>}
  </div>;
}
...
function CarouselPreview({ media }: { media: Media[] }) {
  ...
  return <div className="relative overflow-hidden bg-[#e9e5de]">
    <div className="grid grid-cols-2 gap-2 p-2">{visible.map(item => <img key={item.id} src={mediaUrl(item.path)} alt={item.alt ?? item.filename ?? ""} className="pointer-events-none h-[330px] w-full object-cover sm:h-[400px]" />)}</div>
...
export default function SliderGalleryEditor({ storyId, block, onChange }: Props) {
  ...
  useEffect(() => {
    setDraftIds(ids); setDraftMedia(blockMedia);
    if (!ids.length || blockMedia.length) return;
    let cancelled = false;
    fetch("/api/admin/media").then(response => response.json() as Promise<{ media?: Media[] }>).then(result => {
      if (cancelled) return;
  ...
  const save = async () => {
    ...
    onChange(patch); setManageOpen(false);
    try {
  ...
  return <div className="w-full overflow-hidden rounded-sm">
    <div className="relative">
      {selected.length ? (isCarousel ? <CarouselPreview media={selected} /> : <SlideshowPreview media={selected} />) : <div className="relative overflow-hidden bg-[#e9e5de]"><img src={demo} alt="" className="pointer-events-none block h-auto w-full" /></div>}
      <button type="button" aria-label={selected.length ? "Manage slider images" : "Choose images for slider"} onClick={openManager} className="absolute inset-0 z-[1] h-full w-full cursor-pointer bg-transparent" />
      ...
      {selected.length > 1 && <div className="pointer-events-none absolute inset-0 z-[2]" />}
    </div>
