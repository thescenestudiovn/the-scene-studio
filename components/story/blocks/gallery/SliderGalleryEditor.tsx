...
function ManageImages({ open, media, ids, onChange, onCancel, onDone, onAdd }: ManageProps) {
  ...
  const move = (targetId: string) => {
    ...
    const next = [...ids]; const from = next.indexOf(dragId); const to = next.indexOf(targetId);
...
export default function SliderGalleryEditor({ storyId, block, onChange }: Props) {
  ...
  return <div className="w-full overflow-hidden rounded-sm">
    <button type="button" onClick={openManager} className="block w-full text-left">
      {selected.length ? <div className="relative overflow-hidden bg-[#e9e5de]"><img src={mediaUrl(selected[0].path)} alt={selected[0].alt ?? selected[0].filename ?? ""} className="block h-auto w-full" /><div className="absolute bottom-3 right-3 rounded-full bg-black/55 px-3 py-1.5 text-[10px] uppercase tracking-[.12em] text-white">{selected.length} images · Edit gallery</div></div> : <div className="relative overflow-hidden bg-[#e9e5de]"><img src={demo} alt="" className="block h-auto w-full" /><div className="absolute inset-x-0 bottom-0 bg-black/45 px-4 py-3 text-[10px] uppercase tracking-[.16em] text-white">Choose images for {block.variant === "carousel" ? "carousel" : "slideshow"}</div></div>}
    </button>
