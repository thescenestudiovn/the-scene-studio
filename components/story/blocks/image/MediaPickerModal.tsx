    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[2px]">
      <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex shrink-0 items-center justify-between border-b border-[#ebe7e0] px-6 py-5">
          <div><p className="text-[10px] uppercase tracking-[.18em] text-[#8a857d]">Media Library</p><h2 className="mt-1 font-serif text-2xl text-[#171717]">Choose image{required > 1 ? "s" : ""}</h2></div>
        ...
        <div className="flex shrink-0 gap-2 overflow-x-auto border-b border-[#ebe7e0] px-6 py-3">
          ...
          {collections.map(collection => <button key={collection.id} type="button" onClick={() => setActiveCollection(collection.id)} className={`whitespace-nowrap rounded-full px-3 py-1.5 text-xs ${activeCollection === collection.id ? "bg-[#171717] text-white" : "bg-[#f5f2ed] text-[#625e57]"}`}>{collection.title}</button>)}
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-6">
          {loading ? <div className="flex h-48 items-center justify-center text-sm text-[#99938b]">Loading media…</div> : visibleMedia.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">{visibleMedia.map(item => { const isSelected = selected.includes(item.id); return <button key={item.id} type="button" onClick={() => toggle(item.id)} className={`group relative overflow-hidden rounded-lg border bg-[#f3f0eb] ${isSelected ? "border-[#171717] ring-2 ring-[#171717]/15" : "border-[#e5dfd7]"}`}><img src={mediaUrl(item.path)} alt={item.alt ?? item.filename} className="aspect-square h-auto w-full object-cover"/><span className={`absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full text-xs ${isSelected ? "bg-[#171717] text-white" : "bg-white/85 text-[#77736c]"}`}>{isSelected ? "✓" : ""}</span><span className="absolute inset-x-0 bottom-0 truncate bg-black/45 px-2 py-1.5 text-left text-[10px] text-white">{item.filename}</span></button>; })}</div> : <div className="flex h-48 items-center justify-center rounded-xl border border-dashed border-[#ddd6cd] text-sm text-[#99938b]">No media found.</div>}
        </div>
