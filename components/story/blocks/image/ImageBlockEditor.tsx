...
export default function ImageBlockEditor({ storyId, block, onChange }: Props) {
  ...
  return <div className="relative overflow-visible rounded-sm border border-transparent focus-within:border-[#d9d3ca]"><div className="mb-2"><span className="text-[9px] uppercase tracking-[.16em] text-[#8a857d]">{LABELS[variant] ?? "Image"}</span></div>{variant === "medium" ? <div className="flex justify-center"><div style={{ width: "50%" }}>{renderSlot(0)}</div></div> : variant === "large" ? <div className="flex justify-center"><div style={{ width: "70%" }}>{renderSlot(0)}</div></div> : variant === "full-width" ? renderSlot(0) : <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${required}, minmax(0, 1fr))` }}>{Array.from({ length: required }, (_, i) => renderSlot(i))}</div>}<MediaPickerModal open={pickerOpen} required={1} selectedIds={activeSlot < selectedIds.length && selectedIds[activeSlot] ? [selectedIds[activeSlot]] : []} collectionId={typeof data.collection_id === "string" ? data.collection_id : ""} onClose={() => setPickerOpen(false)} onDone={applySelection} /></div>;
}
