...
export default function ImageWithTextEditor({ storyId, block, onChange }: Props) {
  ...
  return <div className="w-full overflow-hidden rounded-sm border border-transparent focus-within:border-[#d9d3ca]">
    ...
    <div className="mb-4 flex flex-wrap gap-1.5">{VARIANTS.map(option=><button key={option} type="button" onClick={()=>changeLayout(option)} className={`rounded-full px-2.5 py-1.5 text-[9px] uppercase tracking-[.1em] ${variant===option?"bg-[#171717] text-white":"bg-[#f1eee8] text-[#68635b]"}`}>{labelFor(option)}</button>)}</div>
