...
const DEFAULTS: Record<string, string[]> = {
  "columns-1": ["Add your story, a meaningful detail, or a short reflection here."],
  ...
  "text-columns-2": ["This is the first column. Add your story, a meaningful detail, or a short reflection here.", "This is the second column. Continue the story with another detail, memory, or thought here."],
  "text-columns-3": ["First column sample text. Add a short story or detail here.", "Second column sample text. Add another meaningful moment here.", "Third column sample text. Finish this section with another thought here."],
  "text-columns-4": ["First column sample text.", "Second column sample text.", "Third column sample text.", "Fourth column sample text."],
};
...
function columnCount(variant:string){
...
function getColumns(block: StoryBlock): Column[] {
  ...
  const defaults=DEFAULTS[variant]??DEFAULTS[desired===1?"columns-1":desired===2?"columns-2":"columns-3"];
  return Array.from({length:desired},(_,index)=>existing[index]??{content:defaults[index]??"Add your text here."});
}
...
export default function TextColumnsEditor({ block, onChange }: Props) {
  ...
  }, [block.data, block.variant, editingIndex]);
  ...
  const updateColumn = (index: number) => {
    const next = columns.map((column, i) => ({ content: refs.current[i]?.innerHTML ?? column.content }));
