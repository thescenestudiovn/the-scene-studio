...
const SIZE_OPTIONS = [
  { value: "heading-1", label: "Heading 1", tag: "h1", fontSize: "40px", lineHeight: "1.15", fontFamily: "Georgia, serif" },
  { value: "heading-2", label: "Heading 2", tag: "h2", fontSize: "32px", lineHeight: "1.15", fontFamily: "Georgia, serif" },
  { value: "heading-3", label: "Heading 3", tag: "h3", fontSize: "26px", lineHeight: "1.2", fontFamily: "Georgia, serif" },
  { value: "paragraph-1", label: "Paragraph 1", tag: "p", fontSize: "20px", lineHeight: "1.7", fontFamily: "inherit" },
  { value: "paragraph-2", label: "Paragraph 2", tag: "p", fontSize: "16px", lineHeight: "1.7", fontFamily: "inherit" },
  { value: "paragraph-3", label: "Paragraph 3", tag: "p", fontSize: "14px", lineHeight: "1.7", fontFamily: "inherit" },
] as const;
...
function TextToolbar({ editorRef, onChange }: { editorRef: React.RefObject<HTMLDivElement | null>; onChange: () => void }) {
  ...
  const setSize = (value: string) => {
    ...
    const editor = editorRef.current;
    if (!editor || !option || !restoreSelection()) return;
    ...
    if (block instanceof HTMLElement && editor.contains(block)) {
      block.style.fontSize = option.fontSize;
      block.style.lineHeight = option.lineHeight;
      block.style.fontFamily = option.fontFamily;
    }
    saveSelection();
    onChange();
  };
  ...
  return <div className="flex flex-wrap items-center gap-0.5 border-b border-[#d9d3ca] bg-[#f7f4ef] px-2 py-1.5 shadow-sm" onMouseDown={saveSelection}>
    <select aria-label="Text size" title="Text size" defaultValue="paragraph-2" onChange={e => setSize(e.target.value)} className="h-8 w-[118px] border border-[#d9d3ca] bg-white px-2 text-xs text-[#403c36] outline-none hover:border-[#aaa49a] focus:border-[#8f887e]">{SIZE_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
    ...
    <label title="Text color" className="relative flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm text-[#403c36] hover:bg-white" onMouseDown={saveSelection}><span className="relative text-sm font-semibold leading-6 after:absolute after:bottom-0 after:left-0 after:right-0 after:h-1 after:bg-[#7d4f45]">A</span><input aria-label="Text color" type="color" defaultValue="#222222" className="absolute inset-0 cursor-pointer opacity-0" onChange={e => run("foreColor", e.target.value)} /></label>
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    {alignments.map(item => <button key={item.command} type="button" title={item.title} aria-label={item.title} onMouseDown={e => { e.preventDefault(); saveSelection(); }} onClick={() => run(item.command)} className="flex h-8 w-8 items-center justify-center rounded-sm text-[#403c36] hover:bg-white"><AlignIcon align={item.value} /></button>)}
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    <button type="button" title="Bulleted list" aria-label="Bulleted list" onMouseDown={e => { e.preventDefault(); saveSelection(); }} onClick={() => run("insertUnorderedList")} className="flex h-8 w-8 items-center justify-center rounded-sm text-[#403c36] hover:bg-white"><ListIcon ordered={false} /></button>
    <button type="button" title="Numbered list" aria-label="Numbered list" onMouseDown={e => { e.preventDefault(); saveSelection(); }} onClick={() => run("insertOrderedList")} className="flex h-8 w-8 items-center justify-center rounded-sm text-[#403c36] hover:bg-white"><ListIcon ordered /></button>
    <button type="button" title="Remove formatting" aria-label="Remove formatting" onMouseDown={e => { e.preventDefault(); saveSelection(); }} onClick={() => run("removeFormat")} className="flex h-8 w-8 items-center justify-center rounded-sm text-sm font-medium text-[#403c36] hover:bg-white">T<span className="text-[#77736c]">x</span></button>
  </div>;
}
...
function TextBlockEditor({ block, onChange, onBlur }: { block: StoryBlock; onChange: (patch: Partial<StoryBlock>) => void; onBlur: () => void }) {
  ...
  return <div className="overflow-hidden border border-transparent transition-colors focus-within:border-[#d9d3ca]">{editing && <TextToolbar editorRef={editorRef} onChange={changed} />}<div className={isColumns ? "grid gap-8 " + (columns === 4 ? "md:grid-cols-4" : columns === 3 ? "md:grid-cols-3" : "md:grid-cols-2") : ""}><div ref={editorRef} contentEditable suppressContentEditableWarning spellCheck className={`min-h-12 whitespace-pre-wrap px-2 py-2 outline-none ${style}`} onFocus={() => setEditing(true)} onInput={changed} onBlur={() => { setEditing(false); onBlur(); }} /></div></div>;
}
