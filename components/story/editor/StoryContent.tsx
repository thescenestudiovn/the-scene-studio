...
const TOOLBAR = [
  ["bold", "B", "Bold"], ["italic", "I", "Italic"], ["underline", "U", "Underline"], ["justifyLeft", "≡", "Align left"], ["justifyCenter", "≡", "Align center"], ["justifyRight", "≡", "Align right"], ["justifyFull", "≡", "Justify"], ["insertUnorderedList", "•≡", "Bulleted list"], ["insertOrderedList", "1≡", "Numbered list"],
] as const;
...
function TextToolbar({ editorRef, onChange }: { editorRef: React.RefObject<HTMLDivElement | null>; onChange: () => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-0.5 border-b border-[#d9d3ca] bg-[#f7f4ef] px-2 py-1.5 shadow-sm">
    <select aria-label="Font size" title="Font size" defaultValue="3" onChange={e => run("fontSize", e.target.value)} className="mr-1 h-8 border border-[#d9d3ca] bg-white px-2 text-xs outline-none hover:border-[#aaa49a]"><option value="1">8</option><option value="2">10</option><option value="3">12</option><option value="4">14</option><option value="5">18</option><option value="6">24</option><option value="7">32</option></select>
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    {TOOLBAR.slice(0, 3).map(([command, label, title]) => <button key={command} type="button" title={title} aria-label={title} onMouseDown={e => e.preventDefault()} onClick={() => run(command)} className="flex h-8 min-w-8 items-center justify-center border border-transparent px-2 text-sm hover:border-[#d9d3ca] hover:bg-white"><span className={command === "bold" ? "font-bold" : command === "italic" ? "italic" : "underline"}>{label}</span></button>)}
    <label title="Text color" className="relative flex h-8 min-w-8 cursor-pointer items-center justify-center border border-transparent hover:border-[#d9d3ca] hover:bg-white"><span className="relative text-sm font-semibold leading-6 after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-black">A</span><input aria-label="Text color" type="color" className="absolute inset-0 cursor-pointer opacity-0" onChange={e => run("foreColor", e.target.value)} /></label>
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    {TOOLBAR.slice(3, 7).map(([command, label, title], index) => <button key={command} type="button" title={title} aria-label={title} onMouseDown={e => e.preventDefault()} onClick={() => run(command)} className="flex h-8 min-w-8 items-center justify-center border border-transparent px-1 text-base hover:border-[#d9d3ca] hover:bg-white"><span className={index === 0 ? "text-left" : index === 1 ? "text-center" : index === 2 ? "text-right" : "text-justify"}>{label}</span></button>)}
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    {TOOLBAR.slice(7).map(([command, label, title]) => <button key={command} type="button" title={title} aria-label={title} onMouseDown={e => e.preventDefault()} onClick={() => run(command)} className="flex h-8 min-w-8 items-center justify-center border border-transparent px-2 text-xs hover:border-[#d9d3ca] hover:bg-white">{label}</button>)}
    <span className="mx-1 h-5 w-px bg-[#d9d3ca]" />
    <button type="button" title="Clear formatting" aria-label="Clear formatting" onMouseDown={e => e.preventDefault()} onClick={() => run("removeFormat")} className="flex h-8 min-w-8 items-center justify-center border border-transparent px-2 text-sm font-medium hover:border-[#d9d3ca] hover:bg-white">T<span className="text-[#77736c]">x</span></button>
  </div>;
}
...
function BlockCard({ block, blocks, onBlocksChange, onDelete, onUpdate }: { block: StoryBlock; index: number; blocks: StoryBlock[]; onBlocksChange: Props["onBlocksChange"]; onDelete: Props["onDelete"]; onUpdate: Props["onUpdate"] }) {
