...
function BlockEditor({storyId,block,blocks,onBlocksChange,onUpdate}:{storyId:string;block:StoryBlock;blocks:StoryBlock[];onBlocksChange:Props["onBlocksChange"];onUpdate:Props["onUpdate"]}){const updateLocal=(patch:Partial<StoryBlock>)=>onBlocksChange(blocks.map(item=>item.id===block.id?{...item,...patch}:item));const variant=block.variant??"";const isText=block.type==="text"||block.type.startsWith("text-");const isTextColumns=isText&&["columns-1","columns-2","columns-3","text-columns-2","text-columns-3","text-columns-4"].includes(variant);const isGrid=block.type==="image"&&variant.startsWith("grid-");const isImageWithText=block.type==="image"&&variant.startsWith("text-");const isBanner=(block.type==="content"||block.type==="banner")&&variant.startsWith("banner-");if(isTextColumns)return <TextColumnsEditor block={block} onChange={updateLocal}/>;if(isText)return <TextBlockEditor block={block} onChange={updateLocal} onBlur={()=>onUpdate(block,{body:block.body,title:block.title})}/>;if(isGrid)return <GridGalleryEditor storyId={storyId} block={block} onChange={updateLocal}/>;if(isImageWithText)return <ImageWithTextEditor storyId={storyId} block={block} onChange={updateLocal}/>;if(block.type==="image")return <ImageBlockEditor storyId={storyId} block={block} onChange={updateLocal}/>;if(isBanner)return <BannerBlockEditor block={block} onChange={updateLocal}/>;return <div className="border border-dashed border-[#d9d3ca] bg-white p-7 lg:p-9"><p className="text-sm text-[#77736c]">This block type does not have an editor yet.</p></div>}
