...
function TextBlockEditor({ block, onChange, onBlur }: { block: StoryBlock; onChange: (patch: Partial<StoryBlock>) => void; onBlur: () => void }) { const editorRef = useRef<HTMLDivElement | null>(null); const [editing, setEditing] = useState(false); const content = block.body ?? block.title ?? ""; useEffect(() => { if (editorRef.current && !editing && editorRef.current.innerHTML !== content) editorRef.current.innerHTML = content; }, [content, editing]); const changed = () => onChange({ body: editorRef.current?.innerHTML ?? "" }); const variant = block.variant ?? block.type; const style = TEXT_STYLES[variant] ?? "text-base leading-7"; const isColumns = variant.includes("columns-2") || variant.includes("columns-3") || variant.includes("columns-4"); const columns = variant.includes("columns-4") ? 4 : variant.includes("columns-3") ? 3 : 2; return <div className="overflow-hidden border border-transparent transition-colors focus-within:border-[#d9d3ca]">{editing && <TextToolbar editorRef={editorRef} onChange={changed} />}<div className={isColumns ? "grid gap-8 " + (columns === 4 ? "md:grid-cols-4" : columns === 3 ? "md:grid-cols-3" : "md:grid-cols-2") : ""}><div ref={editorRef} contentEditable suppressContentEditableWarning spellCheck className={`min-h-12 whitespace-pre-wrap px-2 py-2 outline-none ${style}`} onFocus={() => setEditing(true)} onInput={changed} onBlur={() => { setEditing(false); onBlur(); }} /></div></div>; }
