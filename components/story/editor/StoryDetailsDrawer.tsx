...
export default function StoryDetailsDrawer(props: Props) {
  ...
  return <>
    <div className="flex justify-end border-b border-[#ddd7cd] bg-[#f5f2ec] px-5 py-3 sm:px-8 lg:px-10">
    ...
    {open && <div className="fixed inset-0 z-50">
      <button aria-label="Close story details" type="button" className="absolute inset-0 bg-black/20" onClick={() => setOpen(false)} />
      <aside className="absolute right-0 top-0 h-[100dvh] w-full max-w-[520px] overflow-y-auto bg-[#f5f2ec] shadow-2xl animate-[slideIn_.25s_ease-out]">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#d9d3ca] bg-[#f5f2ec]/95 px-6 py-5 backdrop-blur">
          <div><p className="text-[10px] uppercase tracking-[0.2em] text-[#8a857d]">Story Settings</p><p className="mt-1 text-sm font-medium">{props.story.title || "Untitled story"}</p></div>
