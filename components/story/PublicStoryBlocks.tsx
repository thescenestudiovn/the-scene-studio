...
function Slider({ media }: { media: Media[] }) {
  ...
  return <div className="relative mx-auto max-w-7xl px-6 md:px-10">
    ...
    {media.length > 1 && <div className="mt-4 flex items-center justify-between text-[10px] uppercase tracking-[0.18em] text-[#77736c]">
      <span>{String(index + 1).padStart(2, "0")} / {String(media.length).padStart(2, "0")}</span>
      <div className="flex gap-1.5">{media.map((item, itemIndex) => <button key={item.id} type="button" aria-label={`Go to image ${itemIndex + 1}`} onClick={() => setIndex(itemIndex)} className={`h-1 w-8 transition ${itemIndex === index ? "bg-[#171717]" : "bg-[#c9c5bc]"}`} />)}</div>
    </div>}
  </div>;
}
...
function Carousel({ media }: { media: Media[] }) {
  ...
  if (!media.length) return null;
  const scroll = (direction: number) => viewportRef.current?.scrollBy({ left: direction * (viewportRef.current.clientWidth * 0.78), behavior: "smooth" });
  return <div className="mx-auto max-w-7xl px-6 md:px-10">
    <div ref={viewportRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {media.map(item => <div key={item.id} className="w-[82%] shrink-0 snap-start md:w-[32%]"><img src={mediaUrl(item.path)} alt={item.alt || item.filename || "The Scene Studio"} className="h-[58vh] w-full object-cover" /></div>)}
    </div>
    {media.length > 1 && <div className="mt-5 flex justify-end gap-2">
...
export default function PublicStoryBlocks({ blocks }: { blocks: Block[] }) {
  return <div>{blocks.map(block => {
    ...
    if (block.type === "image" && (variant === "slideshow" || variant === "carousel")) return <section key={block.id} className="py-16 md:py-24">
    ...
    if (block.type === "text" || block.type.startsWith("text-")) return <section key={block.id} className="px-6 py-20 md:px-10 md:py-32"><div className="mx-auto max-w-4xl">
