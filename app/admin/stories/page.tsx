  ...
  return (
    <main className="min-h-screen bg-[#f7f5f0] px-5 py-8 text-[#171717] md:px-10 md:py-12">
      <div className="mx-auto max-w-7xl">
        ...
        {featured && <section className="mt-9"><Link href={`/admin/stories/${featured.id}`} className="group relative block overflow-hidden bg-[#e9e5de]"><div className="aspect-[2.05/1] min-h-[430px] w-full">{cover(featured) ? <img src={cover(featured)!} alt="" className="h-full w-full object-cover transition-transform duration-1000 group-hover:scale-[1.015]"/> : <div className="flex h-full items-center justify-center text-[10px] uppercase tracking-[0.18em] text-[#9a968e]">No cover image</div>}</div><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"/><div className="absolute inset-x-0 bottom-0 px-7 pb-8 text-white md:px-10 md:pb-10"><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] uppercase tracking-[0.18em] text-white/75"><span>{cats(featured).join(" · ") || "Story"}</span>{(featured.destination_name || featured.location) && <><span>·</span><span>{featured.destination_name || featured.location}</span></>}{featured.date && <><span>·</span><span>{featured.date}</span></>}</div><h2 className="mt-3 max-w-4xl font-serif text-4xl leading-[1.03] tracking-[-0.02em] md:text-6xl">{featured.title}</h2>{featured.description && <p className="mt-4 max-w-2xl text-sm leading-6 text-white/85 md:text-base md:leading-7 line-clamp-3">{featured.description}</p>}<div className="mt-5 flex flex-wrap items-center gap-3 text-[9px] uppercase tracking-[0.15em] text-white/65"><span>{featured.published ? "Published" : "Draft"}</span><span>·</span><span>{featured.block_count ?? 0} {(featured.block_count ?? 0) === 1 ? "block" : "blocks"}</span><span>·</span><span>{featured.image_count ?? 0} {(featured.image_count ?? 0) === 1 ? "image" : "images"}</span></div></div></Link></section>}
        ...
        {filtered.length > 0 && <section className="mt-14"><div className="mb-6 flex items-end justify-between border-b border-[#d8d3ca] pb-4"><div><p className="text-[10px] uppercase tracking-[0.2em] text-[#77736c]">All Stories</p><p className="mt-1 font-serif text-2xl">The Journal</p></div><span className="text-[10px] uppercase tracking-[0.15em] text-[#9a968e]">{filtered.length} {filtered.length === 1 ? "story" : "stories"}</span></div><div className="grid gap-x-7 gap-y-14 md:grid-cols-2">{filtered.map(story => <article key={story.id} className="group"><Link href={`/admin/stories/${story.id}`} className="block"><div className="relative aspect-[4/3] overflow-hidden bg-[#e9e5de]">{cover(story) ? <img src={cover(story)!} alt="" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.025]"/> : <div className="flex h-full items-center justify-center text-[10px] uppercase tracking-[0.18em] text-[#9a968e]">No cover image</div>}<span className="absolute left-3 top-3 bg-white/95 px-2 py-1 text-[9px] uppercase tracking-[0.14em]">{story.published ? "Published" : "Draft"}</span></div><div className="pt-5">{meta(story)}<h2 className="mt-3 font-serif text-3xl leading-[1.08] tracking-[-0.015em]">{story.title}</h2>{story.description && <p className="mt-3 max-w-xl text-sm leading-6 text-[#69645d] line-clamp-3">{story.description}</p>}<div className="mt-5 flex flex-wrap items-center gap-3 text-[9px] uppercase tracking-[0.14em] text-[#99958e]"><span>{story.block_count ?? 0} {(story.block_count ?? 0) === 1 ? "block" : "blocks"}</span><span>·</span><span>{story.image_count ?? 0} {(story.image_count ?? 0) === 1 ? "image" : "images"}</span><span className="ml-1 h-3 w-px bg-[#d0cbc2]"/><span className="text-[#171717]">Edit Story →</span></div></div></Link><button onClick={() => void deleteStory(story)} disabled={deleting === story.id} className="mt-2 text-[9px] uppercase tracking-[0.14em] text-red-700 hover:underline disabled:opacity-40">{deleting === story.id ? "Deleting…" : "Delete"}</button></article>)}</div></section>}
