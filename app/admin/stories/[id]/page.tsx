...
import StoryCover from "../../../../components/story/editor/StoryCover";
...
export default function StoryEditorPage() {
  ...
  useEffect(() => {
    if (!story?.slug) { setSlugAvailable(false); setCheckingSlug(false); return; }
  ...
  async function load() {
    ...
    try {
      ...
      const nextLocations = locationData.locations ?? [];
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Failed to load story"); }
    finally { setLoading(false); }
  }

  function selectedCategoryNames() { return selectedCategoryIds.map(item => categories.find(category => category.id === item)?.name).filter((value): value is string => Boolean(value)); }
  function selectedLocationNames() { return selectedLocationIds.map(item => locations.find(location => location.id === item)?.name).filter((value): value is string => Boolean(value)); }
  ...
  async function createCategory() {
    ...
    try {
      ...
      setSelectedCategoryIds(current => current.includes(data.category!.id) ? current : [...current, data.category!.id]);
      setCategoryInput("");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Failed to create category"); }
  ...
  async function createLocation() {
    ...
    setCreatingLocation(true); setMessage("");
    try {
      ...
      setSelectedLocationIds(current => current.includes(data.location!.id) ? current : [...current, data.location!.id]);
      setLocationInput("");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Failed to create location"); }
  ...
  async function saveStory(published: boolean) {
    ...
    try {
      const response = await fetch(`/api/admin/stories/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        title: story.title, slug: story.slug, location: selectedLocationNames()[0] ?? null, location_ids: selectedLocationIds, date: story.date,
        category: selectedCategoryNames()[0] ?? null, category_ids: selectedCategoryIds, description: story.description, destination_id: story.destination_id ?? null,
        seo_title: story.seo_title ?? null, seo_description: story.seo_description ?? null, cover_media_id: story.cover_media_id ?? null, published,
        tags: story.tags ?? null, featured: !!story.featured, hide_from_search: !!story.hide_from_search, social_media_id: story.social_media_id ?? null,
      }) });
  ...
  return <main className="min-h-screen bg-[#f5f2ec] text-[#171717]">
    <header className="sticky top-0 z-30 border-b border-[#ddd7cd] bg-[#f5f2ec]/95 backdrop-blur"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-5 px-6 py-4 lg:px-10"><div className="flex items-center gap-5"><Link href="/admin/stories" className="text-xs uppercase tracking-[0.16em] text-[#77736c]">← Stories</Link><span className="hidden h-5 w-px bg-[#d5cfc5] sm:block"/><div><p className="text-[10px] uppercase tracking-[0.2em] text-[#8a857d]">Story Editor</p><p className="mt-0.5 max-w-[360px] truncate text-sm font-medium">{story.title || "Untitled story"}</p></div></div><div className="flex items-center gap-2"><span className={`hidden rounded-full px-3 py-1.5 text-[10px] uppercase tracking-[0.15em] sm:inline-flex ${story.published ? "bg-[#e3eadf] text-[#43543c]" : "bg-[#e9e5de] text-[#6f6a61]"}`}>{story.published ? "Published" : "Draft"}</span><a href={`/stories/${story.slug}`} target="_blank" rel="noreferrer" className="hidden border border-[#bdb7ad] px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] md:inline-block">Preview</a><button onClick={() => void saveStory(false)} disabled={saveDisabled} className="border border-[#171717] px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] disabled:opacity-40">Save Draft</button><button onClick={() => void saveStory(true)} disabled={saveDisabled} className="bg-[#171717] px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] text-white disabled:opacity-40">Publish</button></div></div></header>
    <div className="mx-auto max-w-[1500px] px-6 py-8 lg:px-10 lg:py-10">
      {message && <div className="mb-6 border border-[#d8d1c7] bg-white px-5 py-3 text-sm text-[#666158]">{message}</div>}
      <StoryCover story={story} storyId={id} tags={tags} categoryNames={categoryNames} onChange={setStory} />
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_350px]">
        <StoryContent storyId={id} blocks={blocks} onBlocksChange={setBlocks} onDelete={deleteBlock} onUpdate={updateBlock} />
        <StoryDetails story={story} categories={categories} locations={locations} destinations={destinations} selectedCategoryIds={selectedCategoryIds} selectedLocationIds={selectedLocationIds} categoryInput={categoryInput} locationInput={locationInput} creatingCategory={creatingCategory} creatingLocation={creatingLocation} onStoryChange={setStory} onToggleCategory={categoryId => setSelectedCategoryIds(current => current.includes(categoryId) ? current.filter(item => item !== categoryId) : [...current, categoryId])} onToggleLocation={locationId => setSelectedLocationIds(current => current.includes(locationId) ? current.filter(item => item !== locationId) : [...current, locationId])} onCategoryInputChange={setCategoryInput} onLocationInputChange={setLocationInput} onCreateCategory={() => void createCategory()} onCreateLocation={() => void createLocation()} />
      </div>
    </div>
  </main>;
}
