...
const BLOCKS = [
  { group: "Text", items: ["Heading 1", "Heading 2", "Heading 3", "Wide Text", "Regular Text", "Narrow Text", "Text Columns 2", "Text Columns 3", "Text Columns 4", "Quote"] },
  { group: "Image", items: ["Image", "Image Columns", "Image Grid"] },
  { group: "Content", items: ["Text + Image", "Image + Text", "Full Section"] },
  { group: "Links", items: ["Button", "Text Link"] },
  { group: "Video", items: ["Video"] },
  { group: "More", items: ["Blog", "Contact", "Social", "Flex Block"] },
];
...
function mediaUrl(path: string) {
  ...
  // Always use the same-origin proxy. The NAS itself sends
  // Cross-Origin-Resource-Policy: same-origin, which prevents the browser
  // from embedding media.thescenestudio.asia directly on this site.
...
function MediaImage({ media, className = "", alt }: { media: Media; className?: string; alt?: string }) {
  const [src, setSrc] = useState(mediaUrl(media.path));
  const [failed, setFailed] = useState(false);
  useEffect(() => { setSrc(mediaUrl(media.path)); setFailed(false); }, [media.path]);
  if (failed || !src) return <div className={`flex items-center justify-center bg-black/5 text-[8px] uppercase tracking-[.18em] opacity-40 ${className}`}>Image unavailable</div>;
  return <img src={src} alt={alt || media.alt || media.filename} className={className} onError={() => setFailed(true)} />;
}
...
export default function StoryEditorPage() {
  ...
  const load = async () => {
    const [storyRes, mediaRes, destRes] = await Promise.all([
      api(`/api/admin/stories/${id}`),
      api("/api/admin/media"),
  ...
  useEffect(() => {
    void load().catch((e) => setMessage(e instanceof Error ? e.message : "Failed to load story")).finally(() => setLoading(false));
  }, [id]);
  ...
  const save = async (patch: Partial<Story>) => {
    ...
    setSaving(true);
    try {
      const data = await api(`/api/admin/stories/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
  ...
  const normalize = async (items: Block[]) => {
    for (let i = 0; i < items.length; i++) {
      if (items[i].sort_order !== i) {
        await api(`/api/admin/stories/${id}/blocks/${items[i].id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sort_order: i }) });
      }
    }
  };
  ...
  const addBlock = async (label: string, index: number) => {
    setWorking(true);
    try {
      const type: BlockType = label === "Image" ? "image" : label === "Image Columns" || label === "Image Grid" ? "gallery" : label === "Quote" ? "quote" : label === "Video" || label === "Contact" || label === "Social" || label === "Blog" || label === "Button" || label === "Text Link" || label === "Flex Block" ? "credits" : "text";
      ...
      if (!created.block) throw new Error("Failed to create story block");
      const next = [...blocks];
      next.splice(index, 0, created.block);
      setBlocks(next);
      await normalize(next);
      await load();
      setEditing(created.block.id);
      setPicker(null);
  ...
  const updateBlock = async (block: Block, patch: Partial<Block>) => {
    setWorking(true);
    try {
      await api(`/api/admin/stories/${id}/blocks/${block.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
      await load();
      setMessage("All changes saved");
  ...
  const duplicate = async (block: Block, index: number) => {
    ...
    try {
      ...
      if (block.media.length && block.type !== "image") {
        await Promise.all(block.media.map((m, i) => api(`/api/admin/stories/${id}/blocks/${created.block!.id}/media`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ media_id: m.id, sort_order: i }) })));
      }
      const next = [...blocks];
      next.splice(index + 1, 0, { ...created.block, media: block.media });
      await normalize(next);
      await load();
      setEditing(created.block.id);
  ...
  const remove = async (block: Block) => {
    ...
    setWorking(true);
    try {
      await api(`/api/admin/stories/${id}/blocks/${block.id}`, { method: "DELETE" });
      setEditing(null);
      await load();
  ...
  const drop = async (target: number) => {
    ...
    if (from < 0 || from === target) { setDragId(null); return; }
    const next = [...blocks];
    ...
    setBlocks(next);
    setWorking(true);
    try { await normalize(next); await load(); setMessage("Block order saved"); }
    catch (e) { setMessage(e instanceof Error ? e.message : "Failed to reorder blocks"); await load(); }
    finally { setWorking(false); }
  };

  const openMedia = (block: Block) => {
    setMediaPicker(block.id);
    setSelectedMedia(block.media.map((m) => m.id));
    setMediaQuery("");
  };

  const filteredMedia = useMemo(() => {
    const q = mediaQuery.trim().toLowerCase();
    return q ? media.filter((m) => `${m.filename} ${m.alt || ""}`.toLowerCase().includes(q)) : media;
  }, [media, mediaQuery]);

  const applyMedia = async () => {
    if (!mediaPicker) return;
    const block = blocks.find((b) => b.id === mediaPicker);
    if (!block) return;
    setWorking(true);
    try {
      if (block.type === "image") {
        await updateBlock(block, { media_id: selectedMedia[0] || null });
      } else {
        const existing = new Set(block.media.map((m) => m.id));
        const added = selectedMedia.filter((x) => !existing.has(x));
        await Promise.all(added.map((m, i) => api(`/api/admin/stories/${id}/blocks/${block.id}/media`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ media_id: m, sort_order: block.media.length + i }) }))); 
        await load();
      }
      setMediaPicker(null);
      setSelectedMedia([]);
  ...
  const setCover = async () => {
    if (!story || selectedMedia.length === 0) return;
    await save({ cover_media_id: selectedMedia[0] });
    setCoverPicker(false);
    setSelectedMedia([]);
  };

  if (loading) return <main className="min-h-screen bg-[#f7f5f0] px-8 pt-24"><p className="text-[10px] uppercase tracking-[.28em] opacity-40">Loading story editor…</p></main>;
  ...
  const cover = media.find((m) => m.id === story.cover_media_id);

  return <main className="min-h-screen bg-[#f7f5f0] text-[#171717]">
    <header className="sticky top-0 z-40 border-b border-black/10 bg-[#f7f5f0]/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-5 md:px-8"><div className="flex items-center gap-5"><button onClick={() => router.push("/admin/stories")} className="text-[10px] uppercase tracking-[.2em] opacity-50">← Stories</button><span className="h-4 w-px bg-black/10"/><span className="font-serif text-xl">Edit Post</span></div><div className="flex items-center gap-3"><span className="hidden text-[9px] uppercase tracking-[.18em] opacity-40 md:inline">{saving ? "Saving…" : message}</span><button onClick={() => setSettings(true)} className="border border-black/15 px-4 py-2 text-[9px] uppercase tracking-[.18em]">Post Settings</button><button onClick={() => void save({ published: 1 })} className="bg-[#171717] px-5 py-2 text-[9px] uppercase tracking-[.18em] text-white">Publish Post</button></div></div></header>
    <section className="mx-auto max-w-[1280px] px-5 py-10 md:px-8"><div className="mx-auto max-w-[980px]">
      <div className="mb-8 text-center"><div className="text-[10px] uppercase tracking-[.22em] opacity-45">{story.date || "No date"}<span className="mx-2">—</span>{story.category || "Click to select categories"}</div><h1 className="mt-4 font-serif text-5xl tracking-[-.045em] md:text-7xl">{story.title}</h1></div>
      <button onClick={() => { setCoverPicker(true); setSelectedMedia(story.cover_media_id ? [story.cover_media_id] : []); }} className="group mb-8 block w-full overflow-hidden border border-black/10 bg-white/20 text-left">
        {cover ? <MediaImage media={cover} className="max-h-[560px] w-full object-cover transition duration-500 group-hover:scale-[1.01]" /> : <div className="flex min-h-[300px] items-center justify-center text-[9px] uppercase tracking-[.2em] opacity-45">Click to change cover image</div>}
      </button>
      ...
      <div className="mt-16 border-t border-black/10 pt-8"><button onClick={() => setSettings(true)} className="text-[9px] uppercase tracking-[.2em] opacity-45">Click to add tags</button><h3 className="mt-12 font-serif text-2xl">Related Posts</h3></div>
    </div></section>
    {picker !== null && <BlockPicker onClose={() => setPicker(null)} onSelect={(label) => void addBlock(label, picker)} />}
    {settings && <SettingsPanel story={story} destinations={destinations} onClose={() => setSettings(false)} onSave={save} />}
    {(mediaPicker || coverPicker) && <MediaPicker media={filteredMedia} selected={selectedMedia} query={mediaQuery} setQuery={setMediaQuery} toggle={(mid) => setSelectedMedia((s) => s.includes(mid) ? s.filter((x) => x !== mid) : [...s, mid])} close={() => { setMediaPicker(null); setCoverPicker(false); setSelectedMedia([]); }} apply={coverPicker ? () => void setCover() : () => void applyMedia()} cover={coverPicker} />}
  </main>;
}

function AddBar({ onClick }: { onClick: () => void }) { return <button onClick={onClick} className="group flex w-full items-center justify-center py-3"><span className="h-px flex-1 bg-black/0 group-hover:bg-black/10"/><span className="mx-4 flex h-8 w-8 items-center justify-center rounded-full border border-black/10 bg-[#f7f5f0] text-lg opacity-45 transition group-hover:scale-110 group-hover:opacity-100">+</span><span className="h-px flex-1 bg-black/0 group-hover:bg-black/10"/></button>; }

function BlockPicker({ onClose, onSelect }: { onClose: () => void; onSelect: (label: string) => void }) { return <div className="fixed inset-0 z-50 bg-black/25"><div className="absolute right-0 top-0 h-full w-full max-w-[760px] overflow-y-auto bg-[#f7f5f0] shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/10 bg-[#f7f5f0]/95 px-7 py-6 backdrop-blur"><div><p className="text-[9px] uppercase tracking-[.25em] opacity-45">Content Builder</p><h2 className="mt-2 font-serif text-4xl">Add Block</h2></div><button onClick={onClose} className="text-2xl opacity-40">×</button></div><div className="grid gap-8 p-7 md:grid-cols-2">{BLOCKS.map((group) => <section key={group.group}><p className="mb-3 text-[9px] uppercase tracking-[.25em] opacity-45">{group.group}</p><div className="space-y-1">{group.items.map((item) => <button key={item} onClick={() => onSelect(item)} className="flex w-full items-center justify-between border-b border-black/8 px-3 py-4 text-left font-serif text-lg hover:bg-black/[.03]">{item}<span className="text-sm opacity-30">→</span></button>)}</div></section>)}</div></div></div>; }
