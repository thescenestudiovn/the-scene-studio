...
async function ensureGalleryPage() {
  ...
  let page = await db
    .prepare(`SELECT id,slug,title,seo_title,seo_description,published FROM pages WHERE slug='gallery' LIMIT 1`)
    .first<{
      id: string;
      slug: string;
      title: string;
  ...
  if (!page) {
    ...
    await db
      .prepare(`INSERT OR IGNORE INTO pages (id,slug,title,page_type,seo_title,seo_description,published) VALUES (?,'gallery','Gallery','gallery',NULL,NULL,1)`)
      .bind(id)
      .run();
...
async function getGalleryPage() {
  ...
  const block = await db
    .prepare(`SELECT data FROM page_blocks WHERE page_id=? AND type='hero' ORDER BY sort_order ASC LIMIT 1`)
    .bind(page.id)
    .first<{ data: string }>();
  ...
  if (block?.data) {
    try {
      data = JSON.parse(block.data) as { eyebrow?: string; body?: string };
    } catch {
      data = {};
    }
  }
  ...
  return {
    ...page,
    eyebrow: data.eyebrow ?? "Collections",
    description: data.body ?? "Manage the gallery collections and the editorial content shown above them.",
  };
}
...
export async function GET() {
  ...
  try {
    const result = await db
      .prepare(`SELECT ${fields} FROM collections c LEFT JOIN destinations d ON d.id=c.destination_id ORDER BY c.created_at DESC`)
      .all();
    ...
    try {
      ...
      // Do not let Gallery page metadata failure hide the collection grid.
      console.error("GET /api/admin/collections galleryPage error:", error);
    }
...
export async function POST(request: Request) {
  try {
    ...
    await db
      .prepare(`INSERT INTO collections (id,title,slug,description,destination_id,client_name,event_date,seo_title,seo_description,published,cover_media_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(
        id,
        body.title.trim(),
        body.slug.trim(),
    ...
    const collection = await db
      .prepare(`SELECT ${fields} FROM collections c LEFT JOIN destinations d ON d.id=c.destination_id WHERE c.id=?`)
      .bind(id)
      .first();
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.galleryPage) {
      const page = await ensureGalleryPage();
      if (!page) {
        return Response.json({ success: false, error: "Unable to initialize Gallery page" }, { status: 500 });
      }

      await db
        .prepare(`UPDATE pages SET title=?,seo_title=?,seo_description=?,published=1,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
        .bind(
      ...
      await db
        .prepare(`INSERT INTO page_blocks (id,page_id,type,sort_order,data) VALUES ('gallery-hero',?,'hero',0,?) ON CONFLICT(id) DO UPDATE SET page_id=excluded.page_id,type=excluded.type,sort_order=excluded.sort_order,data=excluded.data,updated_at=CURRENT_TIMESTAMP`)
        .bind(page.id, data)
        .run();
    ...
    if (!body.id || !body.title?.trim() || !body.slug?.trim()) {
      return Response.json({ success: false, error: "id, title and slug are required" }, { status: 400 });
    }

    const result = await db
      .prepare(`UPDATE collections SET title=?,slug=?,description=?,destination_id=?,client_name=?,event_date=?,seo_title=?,seo_description=?,published=?,cover_media_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
      .bind(
        body.title.trim(),
        body.slug.trim(),
        body.description || null,
    ...
    try {
      ...
      if (bucket) {
        for (const item of media.results) {
          try {
            const key = new URL(item.path).pathname.replace(/^\//, "");
            if (key) await bucket.delete(key);
          } catch {
            // Ignore invalid media paths and continue deleting the database records.
          }
        }
      }
    } catch {
