...
type CollectionPayload = { id?: string; title?: string; slug?: string; description?: string | null; destination_id?: string | null; client_name?: string | null; event_date?: string | null; seo_title?: string | null; seo_description?: string | null; published?: number | boolean; cover_media_id?: string | null };
type GalleryPagePayload = { title?: string; seo_title?: string | null; seo_description?: string | null; eyebrow?: string; description?: string };
...
async function ensureGalleryPage() {
  ...
  if (!page) {
    ...
    await db.prepare(`INSERT INTO pages (id,slug,title,seo_title,seo_description,published) VALUES (?,'gallery','Gallery',NULL,NULL,1)`).bind(id).run();
    page = await db.prepare(`SELECT id,slug,title,seo_title,seo_description,published FROM pages WHERE id=? LIMIT 1`).bind(id).first<{id:string;slug:string;title:string;seo_title:string|null;seo_description:string|null;published:number}>();
  }
  return page;
}
...
async function getGalleryPage() {
  ...
  if (block?.data) { try { data=JSON.parse(block.data) as {eyebrow?:string;body?:string}; } catch {} }
...
export async function GET() {
  ...
  } catch(error) { console.error("GET /api/admin/collections error:",error); return Response.json({success:false,error:"Failed to fetch collections"},{status:500}); }
}
...
export async function POST(request:Request) {
  ...
  } catch(error) { console.error("POST /api/admin/collections error:",error); return Response.json({success:false,error:"Failed to create collection"},{status:500}); }
}
...
export async function PATCH(request:Request) {
  try {
    ...
    return Response.json({success:true,collection});
  } catch(error) { console.error("PATCH /api/admin/collections error:",error); return Response.json({success:false,error:"Failed to update collection"},{status:500}); }
}
...
export async function DELETE(request:Request) {
  ...
  } catch(error) { console.error("DELETE /api/admin/collections error:",error); return Response.json({success:false,error:"Failed to delete collection"},{status:500}); }
}
