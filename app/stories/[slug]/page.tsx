...
async function getDbStory(slug: string): Promise<DbStoryResult | null> {
    ...
    const story = await db.prepare(`
        ...
        WHERE s.slug = ? AND s.published = 1 LIMIT 1
    ...
    const blocks = await Promise.all((rows.results || []).map(async (block): Promise<DbBlock> => {
        const junctionMedia = await db.prepare(`
            ...
            WHERE sbm.block_id = ? ORDER BY sbm.sort_order ASC
        `).bind(block.id).all<DbMedia>();
        let media = junctionMedia.results || [];
        if (media.length === 0 && block.type === "image" && block.media_id) {
            const directMedia = await db.prepare(`SELECT id, path, filename, alt, width, height, 0 AS sort_order FROM media WHERE id = ? LIMIT 1`).bind(block.media_id).first<DbMedia>();
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return { title, description, alternates: { canonical: canonicalUrl }, openGraph: { title, description, url: canonicalUrl, type: "article", siteName: "The Scene Studio", locale: "en_US", images: cover ? [{ url: mediaUrl(cover) }] : undefined }, twitter: { card: "summary_large_image", title, description, images: cover ? [mediaUrl(cover)] : undefined } };
}
...
export default async function StoryPage({ params }: PageProps) {
    ...
    const sections = dbStory ? dbStory.blocks.map(blockToSection).filter(Boolean) : legacy!.sections;
    ...
    return (
        <main className="min-h-screen bg-[#f7f5f0] text-[#171717]">
            ...
            <section className="relative flex min-h-screen items-end overflow-hidden px-6 pb-10 md:px-10 md:pb-14">
                <div className="absolute inset-0">{coverPath && <Image src={mediaUrl(coverPath)} alt={`${title} wedding in ${location}`} fill priority sizes="100vw" className="object-cover" />}<div className="absolute inset-0 bg-black/20" /></div>
