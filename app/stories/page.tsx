...
async function getDbStorySlugs(): Promise<Set<string>> {
    const db = getDB();
    const result = await db
        .prepare(`SELECT slug FROM stories`)
        .all<DbStorySlug>();
...
export default async function StoriesPage() {
    const [dbStories, dbStorySlugs] = await Promise.all([
        getPublishedStories(),
        getDbStorySlugs(),
    ]);

    const publishedItems = dbStories
        .filter((story) => story.cover_path)
        .map((story) => ({
            slug: story.slug,
            title: story.title,
    ...
    // D1 is authoritative for every slug that exists there.
    // Legacy data is only used for stories that have not been migrated to D1 yet.
    const legacyItems = legacyStories
        .filter((story) => !dbStorySlugs.has(story.slug))
        .map((story) => ({
            slug: story.slug,
            title: story.title,
            location: story.location,
    ...
    const collectionJsonLd = {
        ...
        mainEntity: {
            "@type": "ItemList",
            itemListElement: storyItems.map((story, index) => ({
                "@type": "ListItem",
                position: index + 1,
                name: story.title,
        ...
        <main className="min-h-screen bg-[#f7f5f0] text-[#171717]">
            ...
            <section className="px-6 pb-32 pt-40 md:px-10 md:pb-48 md:pt-52">
                <div className="mx-auto max-w-7xl">
                    ...
                    <p className="mt-10 max-w-xl font-sans text-sm leading-7 text-[#77736c]">
                        A collection of intimate weddings, destination celebrations, and the people who made them unforgettable.
                    </p>
                </div>
            </section>
                            ...
                            <article key={story.slug} className="border-t border-[#d8d3ca] py-16 md:py-24">
                                <div className="mb-8 flex items-start justify-between">
                                    <div>
                                        ...
                                        <h2 className="mt-4 font-serif text-4xl tracking-[-0.03em] md:text-6xl">
                                            <Link href={`/stories/${story.slug}`} className="transition-opacity hover:opacity-60">{story.title}</Link>
                                        </h2>
                                        <p className="mt-3 font-sans text-xs tracking-[0.15em] uppercase text-[#77736c]">{story.location}</p>
                                        {destination && (
                                            <Link href={`/destinations/${destination.country}/${destination.slug}`} className="mt-2 inline-block font-sans text-xs tracking-[0.15em] uppercase transition-opacity hover:opacity-50">
                                                {destination.name} →
                                            </Link>
                                        )}
                                    </div>
                                ...
                                <Link href={`/stories/${story.slug}`} className="group block">
                                    <div className="relative aspect-[16/9] overflow-hidden">
                                        <Image src={mediaUrl(story.coverImage)} alt={`${story.title} — ${story.location}`} fill className="object-cover transition-transform duration-700 group-hover:scale-[1.02]" sizes="100vw" />
                                    </div>
                                    <div className="mt-6 max-w-2xl">
                                        <p className="font-sans text-sm leading-7 text-[#77736c]">{story.description}</p>
                                    </div>
            ...
            <section className="border-t border-[#d8d3ca] px-6 py-32 md:px-10 md:py-48">
                <div className="mx-auto max-w-5xl">
                    <p className="font-serif text-4xl leading-tight tracking-[-0.03em] md:text-7xl">Every celebration<br />has its own story.</p>
                    <Link href="/contact" className="mt-12 inline-block font-sans text-xs tracking-[0.2em] uppercase transition-opacity hover:opacity-50">Tell us about yours →</Link>
                </div>
            </section>
            <Footer />
        </main>
