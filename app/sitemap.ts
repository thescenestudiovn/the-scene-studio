...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const destinationPages = destinations.map((d) => ({
    url: `${baseUrl}/destinations/${d.country}/${d.slug}`,
  ...
  try {
    ...
    const db = env.the_scene_studio_db;
    const collections = await db
      .prepare("SELECT slug FROM collections WHERE published=1")
      .all<SlugRow>();
    const stories = await db
      .prepare("SELECT slug FROM stories WHERE published=1")
      .all<SlugRow>();

    collectionPages = (collections.results ?? []).map((item: SlugRow) => ({
      url: `${baseUrl}/gallery/${item.slug}`,
      changeFrequency: "monthly" as const,
      priority: 0.8,
    }));
    storyPages = (stories.results ?? []).map((item: SlugRow) => ({
