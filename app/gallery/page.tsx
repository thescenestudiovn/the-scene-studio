...
export default async function GalleryPage({ searchParams }: Props) {
  ...
  const [page,result,destinations]=await Promise.all([
    getGalleryPage(),
    db.prepare(`SELECT c.id,c.title,c.slug,c.client_name,d.name AS destination_name,d.slug AS destination_slug,d.country AS destination_country,(SELECT m.path FROM media m WHERE m.id=c.cover_media_id AND m.type='image') AS cover_path,(SELECT m.width FROM media m WHERE m.id=c.cover_media_id AND m.type='image') AS cover_width,(SELECT m.height FROM media m WHERE m.id=c.cover_media_id AND m.type='image') AS cover_height,(SELECT COUNT(*) FROM media m WHERE m.collection_id=c.id AND m.type='image') AS media_count FROM collections c LEFT JOIN destinations d ON d.id=c.destination_id WHERE c.published=1 ${destination?"AND d.slug=?":""} ORDER BY COALESCE(c.event_date,c.created_at) DESC,c.created_at DESC`).bind(...(destination?[destination]:[])).all<CollectionRow>(),
