...
export default async function GalleryPage({ searchParams }: Props) {
  ...
  return <main className="min-h-screen bg-[#f7f5f0] text-[#171717]"><Header light/><section className="px-6 pb-16 pt-36 md:px-10 md:pt-44"><div className="mx-auto max-w-[1500px]"><p className="text-[10px] uppercase tracking-[0.24em] text-[#77736c]">{page.eyebrow}</p><div className="mt-5 flex flex-wrap items-end justify-between gap-8"><div><h1 className="font-serif text-6xl tracking-[-0.05em] md:text-8xl">{page.title}</h1>{activeDestination&&<p className="mt-4 text-[10px] uppercase tracking-[0.18em] text-[#77736c]">{activeDestination.name}</p>}</div><p className="max-w-sm text-sm leading-6 text-[#77736c]">{page.description}</p></div><nav className="mt-12 flex flex-wrap gap-x-7 gap-y-3 border-t border-[#d8d3ca] pt-5 text-[10px] uppercase tracking-[0.18em]"><Link href="/gallery" className={!destination?"font-medium":"hover:opacity-50"}>All</Link>{destinationList.map(d=><Link key={d.id} href={`/gallery?destination=${encodeURIComponent(d.slug)}`} className={destination===d.slug?"font-medium":"hover:opacity-50"}>{d.name}</Link>)}</nav></div></section><section className="px-6 pb-32 md:px-10 md:pb-48"><div className="mx-auto max-w-[1200px]"><div className="grid grid-cols-1 gap-x-7 gap-y-14 sm:grid-cols-2 lg:grid-cols-3">{collections.map((collection)=>{const cover=collection.cover_path?mediaUrl(collection.cover_path):null;return <Link key={collection.id} href={`/gallery/${collection.slug}`} className="group block"><div className="relative w-full overflow-hidden bg-[#ddd8cf]" style={{aspectRatio:"4 / 5"}}>{cover&&<img src={cover} alt={collection.title} className="h-full w-full object-cover transition duration-1000 ease-out group-hover:scale-[1.025]"/>}<div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"/><div className="absolute inset-x-0 bottom-0 flex translate-y-2 items-end justify-between p-5 text-white opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100"><span className="text-[10px] uppercase tracking-[0.16em]">View collection</span><span>→</span></div></div><div className="mt-4 flex items-start justify-between gap-4"><div><h2 className="font-serif text-2xl tracking-[-0.02em]">{collection.title}</h2><p className="mt-1 text-[10px] uppercase tracking-[0.15em] text-[#77736c]">{collection.destination_name??""}</p></div><span className="pt-1 text-[10px] text-[#77736c]">{collection.media_count}</span></div></Link>})}</div></div>{collections.length===0&&<div className="mx-auto max-w-[1200px] border-t border-[#d8d3ca] pt-10 text-sm text-[#77736c]">No collections published for this destination yet.</div>}</section><Footer/></main>;
}
